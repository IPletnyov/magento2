...
class MassDelete extends Action implements HttpPostActionInterface
{
    ...
    public function execute(): ResultInterface
    {
        ...
        try {
            $this->newsMassActionProcessor->execute(NewsMassActionProcessor::PROCESS_TYPE_DELETE);
        } catch (LocalizedException $e) {
            $this->messageManager->addErrorMessage(__('Something went wrong. Please try again latter'));
        }
        $this->messageManager->addSuccessMessage(__('All selected news deleted successfully'));
